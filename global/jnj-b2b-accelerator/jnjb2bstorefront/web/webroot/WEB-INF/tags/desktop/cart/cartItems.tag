			...
			<c:forEach items="${cartData.entries}" var="entry">
				...
				<tr class="cartItem">
					...
					<td headers="header4" class="quantity">
						...
						<form:form id="updateCartForm${entry.product.code}_${entry.entryNumber}" action="${cartUpdateFormAction}" method="post" commandName="updateQuantityForm${entry.entryNumber}">
							...
							<c:choose>
								...
								<c:otherwise>
									...
									<ycommerce:testId code="cart_product_updateQuantity">
										<div id="QuantityProduct_${entry.product.code}_${entry.entryNumber}" class="updateQuantityProduct"></a>
									</ycommerce:testId>
								</c:otherwise>
							</c:choose>
						...
						<c:if test="${entry.updateable}" >
							<ycommerce:testId code="cart_product_removeProduct">
								...
								<a href="#" id="RemoveProduct_${entry.product.code}_${entry.entryNumber}" class="submitRemoveProduct">${iconCartRemove}</a>
							</ycommerce:testId>
						</c:if>
					</td>
					...
					<td headers="header7" class="multidimensional">
						<c:if test="${entry.product.multidimensional}" >
							<ycommerce:testId code="cart_product_updateQuantity">
								<a href="#" id="QuantityProductToggle_${entry.product.code}" class="updateQuantityProduct updateQuantityProduct-toggle">+</a>
							</ycommerce:testId>
						</c:if>
					</td>				 
				</tr>
