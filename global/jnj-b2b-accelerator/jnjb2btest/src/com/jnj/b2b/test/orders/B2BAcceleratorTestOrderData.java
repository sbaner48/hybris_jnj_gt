...
import com.jnj.b2b.test.constants.Jnjb2bTestConstants;
...
public class B2BAcceleratorTestOrderData
{
	...
	public void createSampleOrders()
	{
		...
		placeOrderWithInCustomerPermissions(Jnjb2bTestConstants.PAY_USER_UID, "ORDER_SENT_NOTIFICATION_SENT paid", false);

		placeOrderRequiringApproval(Jnjb2bTestConstants.HISTORICAL_USER_UID, "PENDING_APPROVAL");
		placeOrderForQuoteNegotiation(Jnjb2bTestConstants.HISTORICAL_USER_UID, "PENDING_QUOTE");
		placeOrderAndApproveByB2BApprover(Jnjb2bTestConstants.HISTORICAL_USER_UID, "ORDER_SENT_NOTIFICATION_SENT Approved");
		placeOrderWithInCustomerPermissions(Jnjb2bTestConstants.HISTORICAL_USER_UID,
				"ORDER_SENT_NOTIFICATION_SENT costcenter", true);

		placeReplenishmentOrderInCancelledState(Jnjb2bTestConstants.REPENISHMENT_USER_UID, "REPLENISHMENT_ENDED");
		placeReplenishmentOrderInActiveState(Jnjb2bTestConstants.REPENISHMENT_USER_UID, "REPLENISHMENT_ONGOING");
		placeReplenishmentOrderScheduleForFuture(Jnjb2bTestConstants.REPENISHMENT_USER_UID, "REPLENISHMENT_FUTURE");

		placeOrderQuoteRequestWithMerchantResponse(Jnjb2bTestConstants.QUOTE_USER_UID, "APPROVED_QUOTE");
		placeOrderQuoteRequestWithMerchantResponse(Jnjb2bTestConstants.QUOTE_USER_UID, "REJECTED_QUOTE");
		placeOrderExceedingCreditLimit(Jnjb2bTestConstants.QUOTE_USER_UID, "PENDING_MERCHANT_APPROVAL");
		placeOrderExceedingCreditAndRejected(Jnjb2bTestConstants.QUOTE_USER_UID, "REJECTED_BY_MERCHANT");
	...
	{
		createPaymentInfo(Jnjb2bTestConstants.PAY_USER_UID, "USD", createVisaCardInfo(), createUsBillingInfo());
		createPaymentInfo(Jnjb2bTestConstants.PAY_USER_UID, "USD", createMasterCardInfo(), createGermanyBillingInfo());
	}
			...
			final boolean useCostCenter)
	{
		...
		try
		{
			...
			final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					useCostCenter ? Jnjb2bTestConstants.STANDARD_COSTCENTER : null, null);
	...
	protected String placeOrderForQuoteNegotiation(final String userID, final String purchaseOrderNumber)
	{
		...
		try
		{
			...
			final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					Jnjb2bTestConstants.STANDARD_COSTCENTER, "Please approve request");
	...
	protected String placeOrderExceedingCreditAndRejected(final String userID, final String purchaseOrderNumber)
	{
		...
		try
		{
			...
			final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					Jnjb2bTestConstants.LOWLIMIT_COSTCENTER, null);
	...
	{
		// Lookup the site
		final CMSSiteModel cmsSite = getCmsAdminSiteService().getSiteForId(Jnjb2bTestConstants.POWERTOOLS_SITE);
		// Lookup the customer
			...
			final B2BCustomerModel customer)
	{
		LOG.info(String.format("Creating order for [%s] for site [%s]", customer.getUid(),
				Jnjb2bTestConstants.POWERTOOLS_SITE));
		...
		if (containsOrder(orderList, purchaseOrderNumber))
		{
			LOG.info(String.format("Not creating order because purchaseOrderNumber %s for [%s] for site [%s] already exists. ",
					purchaseOrderNumber, customer.getUid(), Jnjb2bTestConstants.POWERTOOLS_SITE));
	...
	{
		final CMSSiteModel cmsSite = getCmsAdminSiteService().getSiteForId(Jnjb2bTestConstants.POWERTOOLS_SITE);
		final B2BCustomerModel customer = getUserService().getUserForUID(Jnjb2bTestConstants.APPROVER_UID.toLowerCase(),
